...
class DataFrame(object):
    ...
    def _check_replace_inputs(self, subset, to_replace, value):
        ...
        if not isinstance(to_replace, valid_types) and not isinstance(to_replace, dict):
            raise ValueError("to_replace should be a bool, float, int, long, string, list, tuple,"
                             f" or dict. Got {type(to_replace)}")

        if not isinstance(value, valid_types) and value is not None:
            if not isinstance(to_replace, dict):
                raise ValueError("If to_replace is not a dict, value should be a bool, float, int,"
                                 f" long, string, list, tuple or None. Got {type(value)}")
